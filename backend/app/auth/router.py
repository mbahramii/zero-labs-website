...
async def register_request(
    ...
    code = await service.request_register(db, payload.phone)
    return OtpResponse(
...
async def register_verify(
    ...
    return await service.verify_register(
        db, payload.phone, payload.code, payload.password, payload.display_name, _device(request)
    )
...
async def login(
    ...
    return await service.login(db, payload.phone, payload.password, _device(request))
...
async def password_reset_request(
    payload: ResetRequest, db: AsyncSession = Depends(get_db)
) -> OtpResponse:
    """Send a reset OTP without leaking account existence."""
    code = await service.request_reset(db, payload.phone)
    return OtpResponse(
...
async def password_reset_confirm(
    payload: ResetConfirmRequest, db: AsyncSession = Depends(get_db)
) -> MessageResponse:
    """Set a new password and invalidate all sessions."""
    await service.confirm_reset(db, payload.phone, payload.code, payload.new_password)
